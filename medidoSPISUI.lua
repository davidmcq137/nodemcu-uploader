...
function gpioCB(lev, pul, cnt)
   ...
   -- NO! don't have to do this!! gpio.trig(flowMeterPin, "up")
end
...
function execCmd(k,v)
   ...
   elseif k == "CalF" then
      ...
      pulsePerOz = tonumber(v)/10
      gotCalFact = true
   elseif k == "CalE" then
      pulsePerOz = pulsePerOz
   elseif k == "Prs" then
   ...
   elseif k == "PwrOff" then
      print("poweroff") -- this version does not have this (does it???)
   elseif k == "pMAX" then
      opPWM = tonumber(v)
...
function timerCB()
   ...
   if seq % 100 == 0 then
      ...
      sendSPI("Batt", v1*2.3) -- new app assumes differnt volt divider... 1:7.504
